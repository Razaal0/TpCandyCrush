export default class Cookie {
  ...
  selectionnee() {
    let elementSelected = this.htmlImage;
    elementSelected.src = Cookie.urlsImagesSurlignees[this.type]; 
    elementSelected.classList.add("cookies-selected");
  }

  deselectionnee() {
    let elementDeselected = this.htmlImage;
    elementDeselected.src = Cookie.urlsImagesNormales[this.type];
    elementDeselected.classList.remove("cookies-selected");
  }
  ...
  /**
   * Retourne true si le cookie est à une distance de 1
   * Retourne false sinon
   * @param {*} cookie1 
   * @param {*} cookie2 
   * @returns 
   */
  static isSwapDistancePossible(cookie1, cookie2) {
